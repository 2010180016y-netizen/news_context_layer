import { createTextFetcher, runSitemapIngestWorker } from '../apps/jobs/src/index.js';
import {
  ArticleRepository,
  FeedRepository,
...
async function main(): Promise<void> {
  ...
  const result = await runSitemapIngestWorker({
...
void main().catch((error: unknown) => {
  ...
  console.error(`ingest:sitemap failed: ${message}`);
  process.exitCode = 1;
});
