import type { ArticleType, ParserPath } from '@news-context/shared-types';

import { SUPPORTED_HOSTNAME, isSupportedNaverNewsUrl } from '../lib/constants.js';
import type {
  ArticleDomSnapshot,
  ArticleExtractionSnapshot,
  ArticleParseResult,
  ParseConfidenceBucket,
  ParsedArticle
} from './types.js';

interface PartialArticleData {
...
const LOW_CONFIDENCE_THRESHOLDS: Readonly<Record<ParserPath, number>> = {
  jsonld: 0.82,
  meta: 0.78,
  dom: 0.72
};
...
const PUBLISHER_TEXT_SELECTORS = ['.media_end_head_top_logo', '.press_logo', '.media_end_linked_more_point'] as const;
const PUBLISHER_ALT_SELECTORS = ['.media_end_head_top_logo img', '.press_logo img'] as const;
const AUTHOR_SELECTORS = ['.media_end_head_journalist_name', '.byline_s', '[data-author]', '.media_end_head_byline'] as const;
const PUBLISHED_SELECTORS = ['.media_end_head_info_datestamp time', '.media_end_head_info_datestamp [data-date-time]'] as const;
const MODIFIED_SELECTORS = ['.media_end_head_info_datestamp [data-modify-date-time]', '.media_end_head_info_datestamp time'] as const;
const SECTION_SELECTORS = ['.media_end_categorize_item', '.Nitem_link', '.media_end_categorize em', '[data-section-name]'] as const;
const BODY_SELECTORS = ['#dic_area', '#newsct_article', '.newsct_article', '.go_trans._article_content'] as const;
const CORRECTION_SELECTORS = ['#dic_area', '#newsct_article', '.media_end_head_info_datestamp'] as const;
...
function chooseFirst(...values: readonly (string | undefined)[]): string | undefined {
...
function splitAuthorNames(value: string | undefined): string[] {
  if (value === undefined) {
    return [];
  }
...
function inferArticleType(typeHint: string | undefined, section: string | undefined, title: string | undefined): ArticleType | undefined {
...
function truncateBodyExcerpt(value: string | undefined): string | undefined {
...
function buildConfidenceBucket(score: number): ParseConfidenceBucket {
  if (score >= 0.9) {
    return 'high';
  }
...
function collectElementTexts(document: Document, selectors: readonly string[]): string[] {
  const values: string[] = [];
...
function collectAttributeValues(document: Document, selectors: readonly string[], attributeName: string): string[] {
  const values: string[] = [];
...
function collectDateCandidates(document: Document, selectors: readonly string[]): string[] {
  const values: string[] = [];
...
function buildMetaMap(document: Document): Record<string, string> {
...
function createEmptyDomSnapshot(): ArticleDomSnapshot {
  return {
...
function createEmptyLayer(): PartialArticleData {
  return {
    author_names: [],
    keywords: [],
...
function hasAnySnapshotSignal(snapshot: ArticleExtractionSnapshot): boolean {
  return (
    snapshot.json_ld_blocks.length > 0 ||
...
function extractObjectName(value: unknown): string | undefined {
  ...
  if (isRecord(value)) {
    return chooseFirst(
      cleanText(typeof value['name'] === 'string' ? value['name'] : undefined),
      cleanText(typeof value['legalName'] === 'string' ? value['legalName'] : undefined),
      cleanText(typeof value['alternateName'] === 'string' ? value['alternateName'] : undefined)
    );
  }
...
function extractAuthorArray(value: unknown): string[] {
  return uniqueStrings(
    arrayFromUnknown(value).flatMap((entry) => {
...
function collectJsonLdCandidates(parsed: unknown): Record<string, unknown>[] {
...
function isArticleLike(node: Record<string, unknown>): boolean {
...
function extractJsonLdLayer(blocks: readonly string[], baseUrl: string): PartialArticleData {
  ...
  for (const block of blocks) {
    try {
      ...
      for (const node of nodes) {
        ...
        const title = chooseFirst(
          cleanText(typeof node['headline'] === 'string' ? node['headline'] : undefined),
          cleanText(typeof node['name'] === 'string' ? node['name'] : undefined)
        );
        const description = chooseFirst(
          cleanText(typeof node['description'] === 'string' ? node['description'] : undefined),
          cleanText(typeof node['abstract'] === 'string' ? node['abstract'] : undefined)
        );
        const bodyText = chooseFirst(
          cleanText(typeof node['articleBody'] === 'string' ? node['articleBody'] : undefined),
          description
        );
        const canonicalUrl = normalizeUrl(
          chooseFirst(
            cleanText(typeof node['url'] === 'string' ? node['url'] : undefined),
            cleanText(typeof node['mainEntityOfPage'] === 'string' ? node['mainEntityOfPage'] : undefined),
            isRecord(node['mainEntityOfPage'])
        ...
        const layer: PartialArticleData = {
...
function extractMetaLayer(meta: Readonly<Record<string, string>>, baseUrl: string): PartialArticleData {
...
function extractDomLayer(snapshot: ArticleDomSnapshot, baseUrl: string): PartialArticleData {
...
function mergeField<T>(...values: readonly (T | undefined)[]): T | undefined {
  return values.find((value) => value !== undefined);
}

function mergeLayers(
  parserPath: ParserPath,
  jsonLd: PartialArticleData,
  meta: PartialArticleData,
  dom: PartialArticleData
): PartialArticleData {
  const orderedLayers: readonly [PartialArticleData, PartialArticleData, PartialArticleData] =
    parserPath === 'jsonld'
      ? [jsonLd, meta, dom]
      : parserPath === 'meta'
        ? [meta, jsonLd, dom]
  ...
  return {
    ...
    correction_note_detected:
      primaryLayer.correction_note_detected ||
      secondaryLayer.correction_note_detected ||
      tertiaryLayer.correction_note_detected
  };
}

function pickParserPath(jsonLd: PartialArticleData, meta: PartialArticleData): ParserPath {
...
function computeIssues(parserPath: ParserPath, merged: PartialArticleData): string[] {
  const issues: string[] = [];
...
function computeParseConfidence(parserPath: ParserPath, merged: PartialArticleData, issues: readonly string[]): number {
...
function hasPartialStatus(issues: readonly string[]): boolean {
  return issues.some((issue) =>
    ['dom_fallback', 'missing_publisher_name', 'missing_published_at', 'modified_before_published'].includes(issue)
  );
}
...
function createUnsupportedArticleResult(
  url: string,
  parserPath: ParserPath,
  failureReason: 'missing_title' | 'unsupported_host' | 'empty_document',
  issues: readonly string[]
): ArticleParseResult {
  return {
    status: 'unsupported',
...
export function buildArticleExtractionSnapshot(document: Document, rawUrl: string = document.location.href): ArticleExtractionSnapshot {
...
export function parseArticleSnapshot(snapshot: ArticleExtractionSnapshot): ArticleParseResult {
...
export function extractArticle(document: Document, rawUrl: string = document.location.href): ArticleParseResult {
...
export function createEmptyArticleSnapshot(url: string): ArticleExtractionSnapshot {
  return {
    url,
    json_ld_blocks: [],
    meta: {},
