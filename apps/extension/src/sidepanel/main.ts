...
import {
  ...
  consumeCheckoutReturnSignal,
  createCheckoutSessionEntry,
...
import { PANEL_PRIMARY_ROUTES, type PanelPrimaryRoute, resolveInitialRoute } from './routes.js';
import {
...
function isPrimaryRoute(route: SidePanelState['route']): route is PanelPrimaryRoute {
  return PANEL_PRIMARY_ROUTES.includes(route as PanelPrimaryRoute);
}

function renderPrimaryNav(): string {
  const items: readonly { route: PanelPrimaryRoute; label: string }[] = [
...
function getRouteLabel(route: SidePanelState['route']): string {
  switch (route) {
    case 'onboarding':
      return 'Setup';
    case 'loading':
...
function getSupportAvailabilityLabel(status: 'configured' | 'placeholder'): string {
  return status === 'configured' ? '바로 문의' : '안내만 제공';
}

function renderShellMasthead(): string {
...
function renderMainMetrics(): string {
  ...
  const stats = [
    ...
    },
    {
      label: 'Signals',
      value: String(
        resolveResponse.common_signals.entities.length + resolveResponse.common_signals.numbers.length
      ),
...
function renderSupportShortcut(
...
function renderUnsupported(): string {
  ...
  return renderShell(`
    ...
    ${renderSupportShortcut('unsupported_or_error', {
      ...
      description:
        '지원 페이지 안내만으로 해결되지 않으면 article URL, screen state, expected vs actual을 함께 적어 문의해 주세요.'
    })}
  `);
}
...
function renderError(): string {
  ...
  return renderShell(`
    ...
    ${renderSupportShortcut('unsupported_or_error', {
      ...
      description:
        'network, timeout, runtime error는 article URL, occurred_at, screen state를 함께 보내면 재현 확인이 빨라집니다.'
    })}
  `);
}

function renderRelatedArticles(): string {
  const resolveResponse = state.resolveResponse;
...
function renderCurrentActions(): string {
  const resolveResponse = state.resolveResponse;
...
function renderMain(): string {
  ...
  const signalNote =
    resolveResponse.state === 'low_confidence' || resolveResponse.common_signals.confidence < 0.5
      ? '공통 신호 정확도가 낮을 수 있습니다.'
      : null;
  const qualitySupportShortcut =
    resolveResponse.state === 'low_confidence' ||
    ...
    if (state.briefingError !== null) {
      ...
      return `
        ...
        ${renderSupportShortcut('unsupported_or_error', {
          ...
          description:
            '브리핑을 다시 불러오지 못하면 article URL, occurred_at, screen state를 함께 남겨 주세요.'
        })}
      `;
    }
...
function renderSettings(): string {
  const analyticsChecked = state.preferences.analyticsOptIn ? 'checked' : '';
  const supportConfig = readSupportRuntimeConfig();
  const supportChannels = listSupportChannels(supportConfig).filter(
    (channel) => channel.type !== 'b2b_inquiry'
  );
...
function normalizeLaunchTrustCopy(): void {
  if (rootElement === null) {
    return;
  }
  ...
  const supportedPageButtons = Array.from(
    rootElement.querySelectorAll<HTMLButtonElement>('[data-action="open-supported-page"]')
  );
  ...
  if (
    state.unsupportedCopy?.primaryAction.type === 'open_supported_page' &&
...
function setState(nextState: SidePanelState): void {
  state = nextState;
  render();
}

async function refreshAccountData(showLoading: boolean): Promise<void> {
...
async function trackPanelOpened(context: {
  ...
  await trackAnalyticsEvent(
    createPanelApiClient(state.preferences),
    state.preferences,
    state.identity,
    'panel_opened',
    await buildPanelOpenedEventProps(context)
  );
}

async function startResolveFlow(): Promise<void> {
  ...
  try {
    ...
    if (!context.supported || context.url === null) {
      if (state.identity !== null) {
        ...
        await trackAnalyticsEvent(
          ...
          state.identity,
          'article_parsed',
          analyticsBundle.articleParsedProps
        );
        ...
        if (analyticsBundle.parseFailureProps !== null) {
          await trackAnalyticsEvent(
            ...
            state.identity,
            'parse_failure',
            analyticsBundle.parseFailureProps
          );
        }
      }
    ...
    const apiClient = createPanelApiClient(state.preferences);
    const parsedArticle: ParsedArticle = article;
...
async function deferOnboarding(): Promise<void> {
  const nextPreferences = {
    ...state.preferences,
    onboardingCompleted: true
  };
...
async function navigateToRoute(route: PanelPrimaryRoute): Promise<void> {
  setState({
    ...state,
    route,
    noticeMessage: null,
...
function resolvePostCheckoutRoute(): PanelPrimaryRoute {
...
async function openPaywall(
  trigger: SidePanelState['paywallTrigger'],
  message: string | null
): Promise<void> {
  setState({
    ...state,
...
async function handleCheckoutReturn(
  signal: CheckoutReturnSignal,
  trackPurchaseEvent: boolean
): Promise<void> {
  if (state.identity === null) {
    return;
  }
  ...
  setState({
    ...
    paywallMessage:
      signal.result === 'cancel' ? '결제가 취소되어 Free 플랜을 유지합니다.' : null,
    noticeMessage:
...
async function handleBriefingAccess(): Promise<void> {
  if (state.identity === null) {
    return;
  }
...
async function handleSaveWatchlist(): Promise<void> {
...
async function handleDeleteWatchlist(watchlistId: string): Promise<void> {
  if (state.identity === null) {
    return;
  }
...
async function handleToggleWatchlistNotifications(
  watchlistId: string,
  enabled: boolean
): Promise<void> {
  if (state.identity === null) {
...
async function handleFeedback(feedbackType: string): Promise<void> {
  if (state.identity === null || state.resolveResponse === null) {
    return;
  }
...
function handleOpenSupportChannel(channelType: 'beta_user_support' | 'billing_issue' | 'parser_or_resolve_quality' | 'b2b_inquiry'): void {
  const supportConfig = readSupportRuntimeConfig();
...
function handleOpenSupportEntryPoint(entryPoint: SupportEntryPoint): void {
  const supportConfig = readSupportRuntimeConfig();
  const channel = getSupportChannelForEntryPoint(supportConfig, entryPoint);
...
function bindActions(): void {
  ...
  for (const navigationButton of Array.from(rootElement.querySelectorAll<HTMLButtonElement>('[data-action="navigate"]'))) {
  ...
  const analyticsToggle = rootElement.querySelector<HTMLInputElement>('[data-action="toggle-analytics"]');
  ...
  rootElement.querySelector<HTMLButtonElement>('[data-action="start-context"]')?.addEventListener('click', () => {
    void completeOnboarding();
  });

  rootElement.querySelector<HTMLButtonElement>('[data-action="skip-onboarding"]')?.addEventListener('click', () => {
    void deferOnboarding();
  });

  rootElement.querySelector<HTMLButtonElement>('[data-action="retry"]')?.addEventListener('click', () => {
  ...
  rootElement.querySelector<HTMLButtonElement>('[data-action="save-watchlist"]')?.addEventListener('click', () => {
    void handleSaveWatchlist();
  });

  rootElement.querySelector<HTMLButtonElement>('[data-action="open-briefing"]')?.addEventListener('click', () => {
    void handleBriefingAccess();
  });

  rootElement.querySelector<HTMLButtonElement>('[data-action="start-checkout"]')?.addEventListener('click', () => {
    void handleStartCheckout();
  });

  rootElement.querySelector<HTMLButtonElement>('[data-action="open-supported-page"]')?.addEventListener('click', () => {
    window.open(NAVER_NEWS_HOME_URL, '_blank', 'noopener,noreferrer');
  });

  for (const supportButton of Array.from(rootElement.querySelectorAll<HTMLButtonElement>('[data-action="open-support-channel"]'))) {
    supportButton.addEventListener('click', () => {
      const channel = supportButton.dataset['channel'];

      if (
        channel === 'beta_user_support' ||
  ...
  for (const supportButton of Array.from(rootElement.querySelectorAll<HTMLButtonElement>('[data-action="open-support-entry-point"]'))) {
    supportButton.addEventListener('click', () => {
      const entryPoint = supportButton.dataset['entryPoint'];

      if (
        entryPoint === 'billing_paywall' ||
  ...
  for (const feedbackButton of Array.from(rootElement.querySelectorAll<HTMLButtonElement>('[data-action="submit-feedback"]'))) {
  ...
  for (const relatedButton of Array.from(rootElement.querySelectorAll<HTMLButtonElement>('[data-action="open-related"]'))) {
  ...
  for (const deleteButton of Array.from(rootElement.querySelectorAll<HTMLButtonElement>('[data-action="delete-watchlist"]'))) {
  ...
  for (const toggle of Array.from(rootElement.querySelectorAll<HTMLInputElement>('[data-action="toggle-watchlist-notifications"]'))) {
...
function registerCheckoutSignalListener(): void {
...
function registerCheckoutReturnBridgeListener(): void {
