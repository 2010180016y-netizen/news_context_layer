...
export function buildResolveRequestFromParsedArticle(article: ParsedArticle): ResolveRequest {
  return {
    url: article.url,
...
export function formatAuthorLine(authorNames: readonly string[]): string {
...
export function buildResultHelperText(response: ResolveResponse): string | null {
...
): { readonly label: string; readonly tone: 'neutral' | 'warning' } | null {
  if (response.state === 'low_confidence') {
    return {
      label: '낮은 확신',
      tone: 'warning'
...
export function isCurrentClusterSaved(
  clusterId: string | null,
  watchlists: readonly WatchlistItem[]
): boolean {
  if (clusterId === null) {
    return false;
  }
...
export function buildWatchlistUsageLabel(profile: ProfileResponse | null): string {
  if (profile === null) {
    return '저장 한도 확인 중';
  }
...
export function buildPlanLabel(profile: ProfileResponse | null): string {
  if (profile === null) {
    return '플랜 확인 중';
  }
...
export function hasFounderPass(profile: ProfileResponse | null): boolean {
  return profile !== null && profile.plan.code === 'founder_pass' && profile.plan.status === 'active';
}

export function canAccessBriefing(profile: ProfileResponse | null): boolean {
  return profile !== null && profile.limits.briefing_enabled;
}

export function buildBriefingCtaLabel(profile: ProfileResponse | null): string {
  return canAccessBriefing(profile) ? '브리핑 보기' : '브리핑은 Founder Pass에서 열립니다';
}

export function buildPaywallTitle(trigger: PaywallTriggerType | null): string {
  switch (trigger) {
    case 'briefing':
      return '브리핑까지 이어 보려면';
    case 'upgrade_cta':
...
export function buildPaywallDescription(trigger: PaywallTriggerType | null): string {
  switch (trigger) {
    case 'briefing':
