...
interface SupportRuntimeGlobals {
  readonly __NEWS_CONTEXT_CONFIG__?: {
    readonly support?: SupportRuntimeConfig | undefined;
  };
}

const DEFAULT_SUPPORT_RESPONSE_SLA =
...
const SUPPORT_CHANNEL_ORDER: readonly SupportChannelType[] = [
  'beta_user_support',
  'billing_issue',
...
const SUPPORT_ENTRY_POINT_CONFIG: Record<SupportEntryPoint, SupportEntryPointConfig> = {
  billing_paywall: {
    ...
    description:
      '결제 취소, Free 유지, checkout 완료 후 unlock 실패, plan state mismatch 같은 문제를 결제 문의 경로로 안내합니다.',
  ...
  quality_review: {
    ...
    description:
      'low-confidence, stale result, parser fallback, freshness/common signal mismatch가 보이면 기사 URL과 기대 결과를 함께 보내 주세요.',
  ...
  unsupported_or_error: {
    ...
    description:
      'unsupported URL, side panel 열기 실패, onboarding confusion, network or runtime error를 beta 문의 경로에서 받습니다.',
  ...
  b2b_inquiry: {
    ...
    description:
      'sample issue report, daily briefing scaffold, JSON/CSV export, pilot follow-up은 B2B 문의 경로로 분리해 받습니다.',
...
function normalizeSupportUrl(value: unknown): string | null {
...
function buildChannel(type: SupportChannelType, url: string | null): SupportChannelConfig {
  switch (type) {
    case 'beta_user_support':
      return {
        ...
        title: 'Beta 사용 중 문의',
        description:
          '열기, 로딩, unsupported, onboarding, 설정 동작처럼 extension side panel 사용 중 만나는 문제를 받습니다.',
    ...
    case 'billing_issue':
      return {
        ...
        description:
          'checkout, 결제 완료 후 unlock, 취소 후 Free 유지, plan state 반영 문제를 받습니다.',
    ...
    case 'parser_or_resolve_quality':
      return {
        ...
        description:
          'title 누락, low-confidence, related precision, freshness, common signal 품질 문제를 받습니다.',
    ...
    case 'b2b_inquiry':
      return {
        ...
        description:
          'sample issue report, daily briefing scaffold, JSON/CSV export, pilot 미팅 문의를 받습니다.',
...
export function resolveSupportConfig(rawConfig: unknown): SupportConfig {
  ...
  const channels = [
    ...
    buildChannel(
      'parser_or_resolve_quality',
      normalizeSupportUrl(record['qualityIssueUrl'])
    ),
    buildChannel('b2b_inquiry', normalizeSupportUrl(record['b2bInquiryUrl']))
  ] satisfies readonly SupportChannelConfig[];
...
export function readSupportRuntimeConfig(): SupportConfig {
  const globals = globalThis as typeof globalThis & SupportRuntimeGlobals;
  return resolveSupportConfig(globals.__NEWS_CONTEXT_CONFIG__?.support);
}

export function getSupportChannel(
...
export function getSupportEntryPoint(type: SupportEntryPoint): SupportEntryPointConfig {
  return SUPPORT_ENTRY_POINT_CONFIG[type];
}

export function resolveSupportChannelTypeForEntryPoint(
  entryPoint: SupportEntryPoint
): SupportChannelType {
  return getSupportEntryPoint(entryPoint).channelType;
}

export function getSupportChannelForEntryPoint(
  config: SupportConfig,
  entryPoint: SupportEntryPoint
): SupportChannelConfig {
...
export function buildSupportFallbackNotice(
  channel: SupportChannelConfig,
  responseSla: string
): string {
...
export function buildSupportEntryReadinessText(channel: SupportChannelConfig): string {
...
export function buildSupportStatusLabel(config: SupportConfig): string {
...
export function buildSupportChannelSummary(channel: SupportChannelConfig): string {
  if (channel.status === 'configured') {
...
export function buildSupportChannelConfigSummary(channel: SupportChannelConfig): string {
  if (channel.status === 'configured') {
...
export function buildSupportRuntimeSourceSummary(): string {
...
export function listSupportChannels(config: SupportConfig): readonly SupportChannelConfig[] {
